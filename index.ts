    ...
    return new Promise<void>((resolve) => {
      this.currentPage.on("request", async (request) => {
        try {
          ...
          if (!this.totalTitlesCount) {
            const totalTitlesText = await this.currentPage
              .locator(
                '[data-testid="list-page-mc-total-items"] [role="presentation"]'
              )
              .innerText();

            const match = totalTitlesText.match(/\d+/);

            this.totalTitlesCount = match ? parseInt(match[0]) : null;
          }
          // 250 is the maximum count rendered without scrolling
          if (
            idArrLength < 250 ||
            (this.totalTitlesCount &&
              this.idArr.length >= this.totalTitlesCount)
          ) {
            resolve();
            return;
          }
